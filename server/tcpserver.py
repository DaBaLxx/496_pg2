...
def dn(conn):
    ...
    filename = command[3:]
    files = subprocess.run(['ls', '-l'], stdout=subprocess.PIPE)
    if filename in files.stdout:
        file_prop = subprocess.run(['ls', '-l', filename], stdout=subprocess.PIPE)
        file_size = file_prop.stdout.split(' ')[4]
        conn.send(struct.pack('i', int(file_size)))
    else:
        conn.send(struct.pack('i', -1))
